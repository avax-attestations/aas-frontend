    ...
    <>
      ...
      <div className="border rounded">
        <Table>
          ...
          <TableBody>
            {schemas?.map(s => {
              const canEditName = !s.name && s.creator === (walletClient?.account?.address ?? '').toLowerCase();
              return (<TableRow key={s.id}>
                ...
                <TableCell>
                  {canEditName ? (
                    <Button
                      variant="link"
                      onClick={() => router.push(`/attest-with-schema/${NAME_A_SCHEMA_UID}?def-schemaId=${s.uid}`)}
